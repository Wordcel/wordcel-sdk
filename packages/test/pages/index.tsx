...
const Home: NextPage = () => {
  ...
  const createConnection = async () => {
    try {
      if(wallet.publicKey === null ||wallet.connected === false || wallet === undefined || wallet === null){
        return;
      }
      const sdk = new wordcelSDK.SDK(anchorWallet as NodeWallet,connection,opts.preflightCommitment,"localnet",gqlClient);
      const res = await sdk.profile.getProfilefromUser(wallet.publicKey);
      const createConnection = await sdk.connection.createConnection(wallet.publicKey,res[0].publicKey);
  ...
  const updateAPost = async() => {
    try {
      ...
      console.log(getPost)
      const res = await sdk.profile.getProfilefromUser(wallet.publicKey);
      const updatePost = await sdk.post.updatePost(wallet.publicKey,res[0].publicKey,getPost[0].publicKey,"https://gist.githubusercontent.com/abishekk92/10593977/raw/589238c3d48e654347d6cbc1e29c1e10dadc7cea/monoid.md")
  ...
  const createPostforUser = async () => {
    try {
      ...
      const createPost = await sdk.post.createPost(wallet.publicKey,res[0].publicKey,"https://arweave.net/NXsp-pJAOCJ_1nPsc6NzH92Oim3fWeGSvI4mw8N8DSA")
  ...
  const getPostHandler = async() => {
    try {
      ...
      console.log("IN Here")
       const sdk = new wordcelSDK.SDK(anchorWallet as NodeWallet,connection,opts.preflightCommitment,"localnet",gqlClient);
       const inviteAccount = new anchor.web3.PublicKey("GDwGrpZBP6QBFRGeo932uLxYSiz4LPRySBaugX4vxEXm");
